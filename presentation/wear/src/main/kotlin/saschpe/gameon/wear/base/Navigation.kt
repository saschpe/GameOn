...
class TopNavigationDrawerAdapter(
    ...
    override fun getItemText(position: Int) = when (position) {
        ...
        TOP_NAVIGATION_SEARCH_POSITION -> context.getString(CommonR.string.search)
        TOP_NAVIGATION_PROFILE_POSITION -> context.getString(CommonR.string.profile)
        else -> throw IllegalArgumentException("Unsupported position")
    }

    override fun getItemDrawable(position: Int) = when (position) {
        TOP_NAVIGATION_FAVORITES_POSITION -> context.getDrawable(CommonR.drawable.ic_favorite_border_24dp)
        TOP_NAVIGATION_OFFERS_POSITION -> context.getDrawable(CommonR.drawable.ic_whatshot_24dp)
        TOP_NAVIGATION_SEARCH_POSITION -> context.getDrawable(CommonR.drawable.ic_search_24dp)
        TOP_NAVIGATION_PROFILE_POSITION -> context.getDrawable(CommonR.drawable.ic_account_circle_24dp)
        else -> throw IllegalArgumentException("Unsupported position")
    }
